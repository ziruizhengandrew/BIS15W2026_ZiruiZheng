...
homerange %>%
  ...
  labs(title="Numbers in each animal group")
```
Answer: Mammalia is most represented.
...
```{r}
  ...
  labs(title="Numbers in carnivores or herbivores")
```
Answer: More carnivores represented in the data
...
```
Answer: tarandus have the largest homerange size, while hipoliti have the smallest homerange size
...
```{r}
homerange %>% 
  select(trophic.guild,species, common.name, family, mean.hra.m2) %>% 
  filter(trophic.guild =="carnivore") %>% 
  slice_max(mean.hra.m2, n=1)

```
Answer: cheetah of carnivorous mammals have the largest homeranges
...
homerange %>% 
  ...
  labs(title = "Body Mass in log10 (g) vs. Homerange Size in log10 (m^2)")
```
Answer: there is a positive relationship between body mass and home range size
...
homerange %>% 
  ...
  labs(title="Home Range Size in log10 (m^2) by birds and mammals")
...
## 10. Use filter to extract the data for only aves and mammalia. Then compares the homerange sizes of these two groups. Please sort the data by homerange sizes in descending order.
```{r}
homerange %>% 
