...
```{r}
polar_bears_1 <- polar_bears %>% 
  replace_with_na_all(condition = ~ .x %in% c(-999, "", "unknown","NA")) 
```
...
```{r}
# Answer: NAs is represented as unknown; it mainly distributed in mating and denning
```
...
polar_bears_1 %>% 
  group_by(popn, sex) %>% 
  summarise(number_of_samples=n(), .groups = 'keep') %>% 
  arrange(popn) %>% 
  ggplot(aes(x=popn, y=number_of_samples, fill= sex))+
  geom_col(alpha=0.8,position="dodge")+
  labs(title="Numbers of samples from each sex in each population", 
       x=NULL, 
       y="n")+
  coord_flip()
```
...
```{r}
#Answer: Sampling isn't evenly distributed
```
...
polar_bears_1 %>% 
  ...
  labs(title="Numbers of bears were sampled in each year for SB", 
       x=NULL, 
       y="n") +
...
polar_bears_1 %>% 
  ...
  ggplot(aes(x=reorder(year, n), y=n)) +
  geom_col(fill = "#0099f9", alpha=0.8)+
  labs(title="Numbers of bears were sampled in each year for CS", 
       x=NULL, 
       y="n") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1),
        plot.title = element_text(size = 14, face="bold"))+
...
```{r}
polar_bears_1$un_creat_ratio = polar_bears_1$urea_nitrogen * 0.466 / polar_bears_1$creatinine
```
...
```{r}
polar_bears_1 %>% 
 mutate(fasting_state = case_when( un_creat_ratio<= 10 ~ "fasting",
                                   un_creat_ratio > 10 ~ "not fasting"))%>% 
  select(bear_id, un_creat_ratio, fasting_state) %>% 
  mutate(fasting_state = factor(fasting_state, levels=c("fasting","not fasting")))
...
#polar_bears_1 %>% 
#  separate(capture_date, into = c("year","month","day"), sep = "-") %>%
#  mutate(year_capture=as.factor(year))

#   mutate(fasting_state = case_when( un_creat_ratio<= 10 ~ "fasting",
 #                                  un_creat_ratio > 10 ~ "not fasting"))%>% 
#  select(bear_id, un_creat_ratio, fasting_state) %>% 
#  mutate(fasting_state = factor(fasting_state, levels=c("fasting","not fasting"))) %>% 
# summarise(n = n(as.factor(fasting_state))) %>%
#  ggplot(aes(x=year, y=n(fasting_state))) +
...
```{r}
lat_p <- c(62.46,72.70)
long_p <- c(-172.2,-134.8 )
bbox <-make_bbox(long_p,lat_p,f=0.03)
map2 <- get_stadiamap(bbox, maptype = "stamen_terrain", zoom = 7)
ggmap(map2)+
  geom_point(data = polar_bears, mapping = aes(long, lat), size = 0.75, color = "steelblue", alpha = 0.6)+
  labs(x="Longitude", y="Latitude", title="Polar bear location")
```
...
server <- function(input, output, session) {
    output$plot <- renderPlot({
      ...
      labs(title = "Variables by Un_creat_ratio")+
      theme_minimal()
