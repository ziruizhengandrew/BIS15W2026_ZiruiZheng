...
table_B <- read.csv("data/table_B.csv")
```
...
```{r, message = FALSE}
spider_locs <- read.csv("data/spiders_locations.csv") %>% clean_names()
spider_locs
```

2. Join the files using an appropriate key and join type.
```{r}
spider_loca <- full_join(spider_records,spider_locs, by="accession")
```
...
```{r}
write.csv(spider_loca, file="spiders_with_locus", row.names = FALSE)
```
