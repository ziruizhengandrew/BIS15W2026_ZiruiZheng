...
```{r}
# These data are tidy. They are repeated entries in region and concil with longer rows than columns, thus they are in the long format.
```
...
sydneybeaches_long %>%
  ...
  group_by(Site, year) %>%
  summarize(
    mean_enterococci = mean(Enterococci..cfu.100ml., na.rm = TRUE)
  )
...
sydneybeaches_long %>%
  ...
  filter(year == 2013) %>%
  group_by(Site, year) %>%
  summarise(mean_enterococci = mean(Enterococci..cfu.100ml., na.rm = TRUE), .groups = "drop") %>%
  slice_max(mean_enterococci, n = 1)

```
#Answer: Little Bay Beach was the most polluted beach in 2013

10. Explore the data! Do one analysis of your choice that includes a minimum of three lines of code.  
```{r}
# What was the least polluted beach in 2017?
sydneybeaches_long %>%
  separate(Date, into = c("day", "month", "year"), sep = "/") %>%
  filter(year == 2017) %>%
  group_by(Site, year) %>%
  summarise(mean_enterococci = mean(Enterococci..cfu.100ml., na.rm = TRUE),
            .groups = "drop") %>%
  slice_min(mean_enterococci, n = 1)
```
