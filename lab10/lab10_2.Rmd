...
```{r}
p+ geom_point(size = 3, color="black", alpha=0.8, shape=24, fill="lightblue")
```
...
ggplot(df, aes(x, y)) +
  ...
  geom_text(aes(label = shape), vjust = -1, size = 5) #1-14 can't fill
```
...
penguins %>% 
  ...
  geom_bar(position = "dodge")+
   coord_flip()+
  labs(title="Island count", x="Count", y=NULL)
```
