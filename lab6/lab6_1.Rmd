...
penguins %>% 
  ...
  labs(title="Relationship b/w body mass and flipper length")
```
...
```{r}
penguins %>% 
  distinct(species, island,.keep_all=T) ## .? keep_all?
```
...
```{r}
mammals %>% 
  mutate(across(c("order","family"), tolower)) # specific col
```

This will change all columns to lower case. But, notice what happens to numeric columns.
```{r}
mammals %>% 
  mutate(across(everything(), tolower)) #problem: numeric -> character
```
...
```{r}
mammals <- mammals %>% 
  mutate(across(c(mass,gestation, max_life, newborn, weaning, litter_size, afr, litters_year), ~ifelse(.==-999, NA,.)))
```
...
mammals %>% 
  ...
  geom_point()+
  labs(title="relationship b/w gestation and newborn mass")
